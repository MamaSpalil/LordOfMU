    ...
    {    
        switch(Op) // Find Current Opcode
        {
            ...
            case 0x80: case 0x82: case 0x83:// 1 byte
            {
                ...
                if(FOpcode>0x7F) // check for signed numbers!!
                {
                    ...
                    sprintf(temp,"%s%02X",Scale[0],FOpcode); // '-' aritmethic                    
				}
                else
                    sprintf(temp,"%02X",FOpcode);

                // Read Opcodes: Opcode - imm8
                sprintf(m_Bytes,"%02X%04X",Op,wOp);
            ...
            case 0x81: case 0xC7: case 0x69: // 2 (WORD)/4 (DWORD) bytes
            {
                ...
                else // READ DWORD
                {
                    ...
                    sprintf(temp,"%08X",dwMem);                    
                    // Read Opcodes: Opcode - imm32
                    sprintf(m_Bytes,"%04X %08X",wOp,dwOp);
        ...
        if(Op==0xC7)
        {
          ...
          if(((m_Opcode&0x38)>>3)!=0) // check 000
              strcat((*Disasm)->Remarks,"Invalid Instruction");

          sprintf(assembly,"%s %s, %s","mov",regs[RM][reg1],temp);
        }
        else{        
            // Build assembly
            if(Op==0x69)
            {
                ...
                sprintf(assembly,"imul %s, %s, %s",regs[RM][reg2],regs[RM][reg1],temp);
            }
            else
                sprintf(assembly,"%s %s, %s",Instructions[REG],regs[RM][reg1],temp);
        }
        
        strcat((*Disasm)->Assembly,assembly);
        ...
        if(Op==0x8C || Op==0x8E)
        {
            ...
            reg1=(m_Opcode&0x07);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(m_Bytes,"%04X",wOp);
            
            if(REG<=5) // SEG IS KNOWN
            {
                if(d==0) // (->) Direction
                {
                    sprintf(assembly,"%s %s, %s",instruction,regs[RM][reg1],segs[REG]);
                }
                else // (<-) Direction
                {
                    sprintf(assembly,"%s %s, %s",instruction,segs[REG],regs[RM][reg1]);
                }
            }
            else // UNKNOWN SEG (NOT IN RANGE 0-5)
            {
                if(d==0) // (->) Direction
                {                
                    sprintf(assembly,"%s %s, SEG ??",instruction,regs[RM][reg1]);
                }
                else //(<-) Direction
                {
                    sprintf(assembly,"%s SEG ??,%s",instruction,regs[RM][reg1]);
                }

                // Put warning
                strcat((*Disasm)->Remarks,"Unknown Segment Used,");
            }
            ...
            if(Op==0x8E) 
                strcat((*Disasm)->Remarks,"Segment Is Being Modified!");
		...
		if(Op==0xC6)
		{
			...
			if(m_Opcode>=0xC0 && m_Opcode<=0xC7)
			{
				...
				SwapWord((BYTE*)(*Opcode+Pos+1),&wOp,&wMem);
				// Read imm16
				sprintf(temp,"%02X",*((BYTE*)(*Opcode+Pos+2)));
				sprintf(m_Bytes,"C6 %04X",wOp);
				// Read Opcodes: Opcode - imm16                   
        ...
        if(Op==0xC0 || Op==0xC1)
        {
            ...
            reg1=(m_Opcode&7); // Get Destination Register
            SwapWord((BYTE*)(*Opcode+Pos+1),&wOp,&wMem);
            sprintf(temp,"%02X",wOp&0x00FF);            
            // Read Opcodes: Opcode - imm8
            sprintf(m_Bytes,"%02X%04X",Op,wOp);
            m_OpcodeSize=3;
            (*(*index))+=2; // Prepare to read next Instruction
            // Build assembly
            sprintf(assembly,"%s %s, %s",ArtimaticInstructions[REG],regs[RM][reg1],temp);
            strcat((*Disasm)->Assembly,assembly);
            (*Disasm)->OpcodeSize=m_OpcodeSize;
            strcat((*Disasm)->Opcode,m_Bytes);
       ...
       switch(Op) 
       {
         case 0x6B: // IMUL REG,REG,IIM
         {
             ...
             if(FOpcode>0x7F) // check for signed numbers!!
             {
                 ...
                 sprintf(temp,"%s",Scale[0]); // '-' aritmathic (Signed)                            
             }
             else                    
                strcpy(temp,"");
             ...
             m_OpcodeSize=3;
             (*(*index))++;
             sprintf(assembly,"imul %s,%s,%s%02X",regs[RM][reg1],regs[RM][reg2],temp,FOpcode);
             sprintf(temp,"%02X%04X",Op,wOp);
         }
         break;
         ...
         case 0x8F: // POP REG
         {
           ...
           sprintf(assembly,"%s %s",instruction,regs[RM][reg2]);
         }
         break;

         case 0xD0: case 0xD1: 
         {
             sprintf(assembly,"%s %s, 1",ArtimaticInstructions[REG],regs[RM][reg1]);
         }
         break;

         case 0xD2: case 0xD3: 
         {
             sprintf(assembly,"%s %s, cl",ArtimaticInstructions[REG],regs[RM][reg2]);
         }
         break;
         ...
         case 0xD8:// FPU Instruction
         {
             ...
             {
                 sprintf(assembly,"%s %s",FpuInstructions[REG],FpuRegs[reg1]);
             }
             else // st(0) is the dest
             {
                 sprintf(assembly,"%s st,%s",FpuInstructions[REG],FpuRegs[reg1]);
             }
         }
         break;
         ...
         case 0xD9: // FPU Instructions
         {
             ...
             switch((BYTE)(*(*Opcode+Pos+1)))
             {
                ...
                {
                    sprintf(assembly,"fxch %s",FpuRegs[reg1]);
                }
                break;
                ...
                {
                   sprintf(assembly,"fst %s",FpuRegs[reg1]);
                }
                break;
                ...
                {
                   sprintf(assembly,"fstp %s",FpuRegs[reg1]);
                }
                break;

                case 0xE2:case 0xE3:case 0xE6:case 0xE7:
                {
                   sprintf(assembly,"fldenv %s",FpuRegs[reg1]);
                }
                break;

                case 0xEF:
                {
                   sprintf(assembly,"fldcw %s",FpuRegs[reg1]);
                }
                break;
				...
				{
					sprintf(assembly,"fld %s",FpuRegs[reg1]);
				}
                break;
         ...
         case 0xDA: // FPU Instructions
         {
             switch((BYTE)(*(*Opcode+Pos+1)))
             {
                ...
                {
                    sprintf(assembly,"fcmovb st,%s",FpuRegs[reg2]);
                }
                break;
                ...
                case 0xCC:case 0xCD:case 0xCE:case 0xCF:
                {
                   sprintf(assembly,"fcmove st,%s",FpuRegs[reg2]);
                }
                break;
                ...
                {
                   sprintf(assembly,"fcmovbe st,%s",FpuRegs[reg2]);
                }
                break;
                ...
                case 0xDC:case 0xDD:case 0xDE:case 0xDF:
                {
                   sprintf(assembly,"fcmovu st,%s",FpuRegs[reg2]);
                }
                break;

                // Default Signed FPU Instructions
                default: sprintf(assembly,"%s %s",FpuInstructionsSigned[REG],FpuRegs[reg2]); break;
             }
         }
         break;
         ...
         case 0xDB: // FPU Instruction
         {
             switch((BYTE)(*(*Opcode+Pos+1)))
             {
                 ...
                 {
                     sprintf(assembly,"fcmovnb st,%s",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 {
                     sprintf(assembly,"fcmovne st,%s",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 {
                     sprintf(assembly,"fcmovnbe st,%s",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 {
                     sprintf(assembly,"fcmovnu st,%s",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 {
                     sprintf(assembly,"fucomi st,%s",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 {
                     sprintf(assembly,"fcomi st,%s",FpuRegs[reg2]);
                 }
                 break;

                 default: sprintf(assembly,"fstp %s",FpuRegs[reg2]); break;
             }
         }
         break;
         ...
         {                          
            ...
            {                
              ...
              sprintf(assembly,"%s %s,st",FpuInstructions[REG],FpuRegs[reg1]);
            }
         }
         break;
         ...
         case 0xDD: // FPU Instruction
         {
             ...
             {             
                ...
                case 0xC4:case 0xC5:case 0xC6:case 0xC7: 
                {
                   sprintf(assembly,"ffree %s",FpuRegs[reg1]);
                }
                break;
                ...
                case 0xDC:case 0xDD:case 0xDE:case 0xDF:
                {
                  sprintf(assembly,"%s %s",FpuInstructionsSet2[REG],FpuRegs[reg1]);
                }
                break;
                ...
                {
                  sprintf(assembly,"fucom %s",FpuRegs[reg1]);
                }
                break;
                ...
                case 0xEC:case 0xED:case 0xEE:case 0xEF: 
                {
                  sprintf(assembly,"fucomp %s",FpuRegs[reg1]);
                }
                break;
                ...
                {
                   sprintf(assembly,"%s %s",FpuInstructionsSet3[REG],FpuRegs[reg1]);
                }
                break;                
             }             
         }
         ...
         case 0xDE: // FPU Instruction
         {
             switch((BYTE)(*(*Opcode+Pos+1)))
             {
                 ...
                 case 0xC4:case 0xC5:case 0xC6:case 0xC7: 
                 {
                     sprintf(assembly,"faddp %s,st",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 case 0xCC:case 0xCD:case 0xCE:case 0xCF: 
                 {                     
                     sprintf(assembly,"fmulp %s,st",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 case 0xD4:case 0xD5:case 0xD6:case 0xD7: 
                 {
                   sprintf(assembly,"ficom %s",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 case 0xDC:case 0xDD:case 0xDE:case 0xDF:
                 {
                   sprintf(assembly,"ficomp %s",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 case 0xE4:case 0xE5:case 0xE6:case 0xE7: 
                 {
                   sprintf(assembly,"fsubrp %s,st",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 case 0xEC:case 0xED:case 0xEE:case 0xEF: 
                 {
                   sprintf(assembly,"fsubp %s,st",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 case 0xF4:case 0xF5:case 0xF6:case 0xF7: 
                 {
                   sprintf(assembly,"fdivrp %s,st",FpuRegs[reg2]);
                 }
                 break;
                 ...
                 case 0xFC:case 0xFD:case 0xFE:case 0xFF: 
                 {
                   sprintf(assembly,"fdivp %s,st",FpuRegs[reg2]);
                 }
                 break;
             }
         }
         ...
         case 0xDF: // FPU Instruction
         {
             switch((BYTE)(*(*Opcode+Pos+1)))
             {
                ...
                case 0xC4:case 0xC5:case 0xC6:case 0xC7: 
                {
                   sprintf(assembly,"ffreep %s",FpuRegs[reg2]);
                }
                break;
                ...
                case 0xD4:case 0xD5:case 0xD6:case 0xD7: 
                {
                   sprintf(assembly,"fist %s",FpuRegs[reg2]);
                }
                break;
                ...
                case 0xDC:case 0xDD:case 0xDE:case 0xDF:
                {
                  sprintf(assembly,"fistp %s",FpuRegs[reg2]);
                }
                break;
                ...
                case 0xE4:case 0xE5:case 0xE6:case 0xE7: 
                {
                  sprintf(assembly,"fbld %s",FpuRegs[reg2]);
                }
                break;
                ...
                case 0xEC:case 0xED:case 0xEE:case 0xEF: 
                {
                   sprintf(assembly,"fucomip st,%s",FpuRegs[reg2]);
                }
                break;
                ...
                case 0xF4:case 0xF5:case 0xF6:case 0xF7: 
                {
                  sprintf(assembly,"fcomip st,%s",FpuRegs[reg2]);
                }
                break;
         ...
         {             
             ...
             {
                 SwapWord((BYTE*)(*Opcode+Pos+1),&wOp,&wMem);
                 sprintf(assembly,"%s %s,%02X",InstructionsSet2[REG],regs[RM][reg2],wOp&0x00FF);
                 (*(*index))++;
                 m_OpcodeSize++;
                 sprintf(m_Bytes,"%02X",wOp&0x00FF);
                 strcat(temp,m_Bytes);
             }
             else
                 sprintf(assembly,"%s %s",InstructionsSet2[REG],regs[RM][reg2]);
         }
         break;
         ...
         {             
           if(reg1==0 || reg1==1)
           {
               ...
               {
                 SwapDword((BYTE*)(*Opcode+Pos+2),&dwOp,&dwMem);
                 sprintf(assembly,"%s %s,%08X",InstructionsSet2[REG],regs[RM][reg2],dwMem);                 
                 sprintf(m_Bytes," %08X",dwOp);
                  (*(*index))+=4; 
                  m_OpcodeSize+=4;
               }
               ...
               {
                   SwapWord((BYTE*)(*Opcode+Pos+2),&wOp,&wMem);
                   sprintf(assembly,"%s %s,%04X",InstructionsSet2[REG],regs[RM][reg2],wMem);                   
                   sprintf(m_Bytes," %04X",wOp);                   
                   (*(*index))+=2;
                   m_OpcodeSize+=2;
               }
         ...
         {
             sprintf(assembly,"%s %s",InstructionsSet3[REG],regs[RM][reg2]);
             if(REG>1)
                 strcat((*Disasm)->Remarks,"Illegal Instruction");
         }
         break;
         
         case 0xFF:
         {
             sprintf(assembly,"%s %s",InstructionsSet4[REG],regs[RM][reg2]);
		 ...
		 case 0x8D:{
			sprintf(assembly,"%s %s, %s",instruction,regs[RM][reg2],regs[RM][reg1]);
			strcat((*Disasm)->Remarks,"Illegal Instruction");
		 }
		 break;
         
         // Default General Instructions
         default: sprintf(assembly,"%s %s, %s",instruction,regs[RM][reg1],regs[RM][reg2]); break;
       }

       strcat((*Disasm)->Assembly,assembly);
       (*Disasm)->OpcodeSize=m_OpcodeSize;
       strcat((*Disasm)->Opcode,temp);
		 ...
		 )
{
	...
	*/
	...
	if(PrefixReg==1)
	{
        ...
        { 
		   if(strcmp(RSize,"Byte")!=0) // doesn't affect byte mode
		   {
	...
	*/
	...
	{		
		...
		switch(FOpcode)
		{
		  case 0x00: case 0x08: sprintf(Addr,"%s",addr16[0]); /*SEG=SEG_DS;*/ break; // Mode 0:[BX+SI]
		  case 0x01: case 0x09: sprintf(Addr,"%s",addr16[1]); /*SEG=SEG_DS;*/ break; // Mode 1:[BX+DI]
		  case 0x02: case 0x0A: sprintf(Addr,"%s",addr16[2]); SEG=SEG_SS; break; // Mode 2:[BP+SI]
		  case 0x03: case 0x0B: sprintf(Addr,"%s",addr16[3]); SEG=SEG_SS; break; // Mode 3:[BP+DI]
		  case 0x04: case 0x0C: sprintf(Addr,"%s",addr16[4]); /*SEG=SEG_DS;*/ break; // Mode 4:[SI]
		  case 0x05: case 0x0D: sprintf(Addr,"%s",addr16[5]); /*SEG=SEG_DS;*/ break; // Mode 5:[DI]
		  case 0x06: case 0x0E: // Mode 6: [BP+XX/XXXX] | [XX]
		  {
				  if(Extension==0) // 0x00-0x3F only! has special [XXXX]
				  {
					  /*SEG=SEG_DS;*/
                      SwapWord((BYTE*)(*Opcode+pos+2),&wOp,&wMem);
					  sprintf(Addr,"%04X",wMem);
					  (*(*index))+=2; // read 2 bytes
				  }
				  else{ // 0x50-0xBF has [BP+]

					  SEG=SEG_SS; // SS Segment
					  sprintf(Addr,"%s",addr16[7]);
				  }
		  }
		  break;
          
		  case 0x07: case 0x0F: sprintf(Addr,"%s",addr16[6]); /*SEG=SEG_DS;*/ break; // Mode 7: [BX]
		}
		...
		switch(Extension)
		{
			...
			{
				sprintf(tempMeme,"%s ptr %s:[%s]",RSize,segs[SEG],Addr);
				SwapDword((BYTE*)(*Opcode+pos),&dwOp,&dwMem);
				SwapWord((BYTE*)(*Opcode+pos),&wOp,&wMem);

                if(((wOp&0x00FF)&0x0F)==0x06) // 0x00-0x3F with mode 6 only!
				{	                    
					sprintf(menemonic,"%08X",dwOp);
					(*Disasm)->OpcodeSize=4;
					strcat((*Disasm)->Opcode,menemonic);
				}
				else{ // other modes                    
					sprintf(menemonic,"%04X",wOp);
					(*Disasm)->OpcodeSize=2;
					strcat((*Disasm)->Opcode,menemonic);
				}
			}
			...
			case 1: // 1 Byte Extension to regMem
			{
				...
				if(FOpcode>0x7F) // check for signed numbers
				{
					sprintf(Aritmathic,"%s",Scale[0]); // '-' Signed Numbers
					FOpcode = 0x100-FOpcode; // -XX
				}
				sprintf(menemonic,"%02X%04X",Op,wOp);
				strcat((*Disasm)->Opcode,menemonic);
				sprintf(tempMeme,"%s ptr %s:[%s%s%02X]",RSize,segs[SEG],Addr,Aritmathic,FOpcode);
				++(*(*index)); // 1 byte read
				(*Disasm)->OpcodeSize=3;
			}
			break;
			...
			case 2: // 2 Bytes Extension to RegMem
			{
				...
				(*Disasm)->OpcodeSize=4;
				strcat((*Disasm)->Opcode,menemonic);
				sprintf(tempMeme,"%s ptr %s:[%s%s%04X]",RSize,segs[SEG],Addr,Aritmathic,wMem);
		...
		switch(Bit_d)
		{
			case 0: // (->)
			{
				...
				switch(Op)// Check for all Cases
				{
                    case 0x6B:
                    {
                        ...
                        {
                            FOpcode = 0x100-FOpcode; // -XX (Signed)
                            sprintf(Aritmathic,"%s",Scale[0]); // '-' aritmathic (Signed)                            
                        }
                        else                    
                            strcpy(Aritmathic,"");
						
					    strcpy(instruction,"imul");
						sprintf(temp,"%s %s,%s,%s%02X",instruction,regs[RM][reg2],tempMeme,Aritmathic,FOpcode);
                    ...
                    case 0x81: case 0xC7: case 0x69:
					{
                        ...
                        if(Op==0xC7)
                        {
                            ...
                            */
                            ...
                            sprintf(instruction,"%s","mov");
                        }
                        else
                        {
                            if (Op==0x69) // IMUL REG,MEM,IIM32
                            {
                                sprintf(instruction,"imul %s,",regs[RM][reg1]);
                            }
                            else
                                sprintf(instruction,"%s",Instructions[REG]);
                        }
                        sprintf(menemonic,"%s %s,%s",instruction,tempMeme,temp);
                    ...
                    case 0x80:case 0x82: case 0x83: case 0xC6:
					{
						...
						sprintf(temp,"%s %s,%s%02X",instruction,tempMeme,Aritmathic,FOpcode);
                    ...
                    {
                        sprintf(temp,"%s %s,%s",instruction,tempMeme,segs[REG]);
                    }
                    break;

                    case 0xD0: case 0xD1:
                    {
                        sprintf(temp,"%s %s,1",ArtimaticInstructions[REG],tempMeme);
                    }
                    break;

                    case 0xD2: case 0xD3:
                    {
                        sprintf(temp,"%s %s,cl",ArtimaticInstructions[REG],tempMeme);
                    }
                    break;

                    case 0xD8: case 0xDC:// Unsigned FPU Instructions (unsigned)
                    {
                        sprintf(temp,"%s %s",FpuInstructions[REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xD9: // FPU Instructions Set2 (UnSigned)
                    {
                        ...
                        sprintf(temp,"%s %s",FpuInstructionsSet2[REG],tempMeme);
                    }
                    break;

                    case 0xDA: case 0xDE:// FPU Instructions (Signed)
                    {
                        sprintf(temp,"%s %s",FpuInstructionsSigned[REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xDB: // FPU Instructions Set2 (Signed)
                    {
                       ...
                       sprintf(temp,"%s %s",FpuInstructionsSet2Signed[REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xDD:// FPU Instructions Set2 (Signed)
                    {
                       ...
                       sprintf(temp,"%s %s",FpuInstructionsSet3[REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xDF: // Extended FPU Instructions Set2 (Signed)
                    {
                       ...
                       sprintf(temp,"%s %s",FpuInstructionsSet2Signed_EX[REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xF6:
                    {
						...
						strcpy(Aritmathic,"");
  				        sprintf(instruction,"%s",InstructionsSet2[REG]);
                        ...
                        {                        
						   ...
						   (*Disasm)->OpcodeSize++;
                            ...
                            sprintf(temp,"%s %s",instruction,tempMeme);
                    }
                    break;
                    ...
                    case 0xF7:
                    {
                        ...
                        {                        
                            ...
                            sprintf(menemonic,"%s %s",instruction,tempMeme);
                    ...
                    {                                                
                      sprintf(temp,"%s %s",InstructionsSet3[REG],tempMeme);
                    ...
                    {
                       sprintf(temp,"%s %s",InstructionsSet4[REG],tempMeme);
                            
                       if(REG==3)// FAR CALL
                       {
                           strcat((*Disasm)->Remarks,"Far Call");
                           break;
                       }
                            
                       if(REG==5) // FAR JUMP
                       {
                          strcat((*Disasm)->Remarks,"Far Jump");
                          break;
                       }
                    ...
                    break;
  			        ...
  			        default:
					{       
					   sprintf(temp,"%s %s,%s",instruction,tempMeme,regs[RM][REG]);                            
					}
					break;
				}
				
				strcat((*Disasm)->Assembly,temp);
				/*
				sprintf(menemonic,"%s %s,%s",instruction,tempMeme,regs[RM][REG]);
			...
			case 1: // (<-) Direction (Bit_D)
			{
                ...
                switch(Op)
                {
                    ...
                    {
                        sprintf(menemonic,"%s %s,%s",instruction,segs[REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xC0: case 0xC1:
                    {
                        ...
                        switch(Extension)
                        {
                            case 0: // No Extension
                            {
                                SwapWord((BYTE*)(*Opcode+pos+1),&wOp,&wMem);
                                FOpcode=wOp&0x00FF;
                                sprintf(menemonic,"%s %s,%02X",ArtimaticInstructions[REG],tempMeme,FOpcode);                                
                                sprintf(tempMeme," %02X",FOpcode);
                                strcat((*Disasm)->Opcode,tempMeme);
                                (*(*index))++;
                                (*Disasm)->OpcodeSize++;
                            }
                            break;
                    ...
                    case 0xC4: // LES
                    {
                      ...
                      sprintf(menemonic,"%s %s,%s",instruction,regs[RM][REG],tempMeme);
                    }
                    break;
				...
				strcat((*Disasm)->Assembly,tempMeme);
			}
			break;
		}
		...
		switch(Extension) // Check what extension we have (None/Byte/Dword)
		{
			case 00: // no extention to regMem
			{
				if(reg1==REG_EBP) // cannot display EBP as memoryReg, use DWORD mem location
				{
                    SwapDword((BYTE*)(*Opcode+pos+2),&dwOp,&dwMem);
                    SwapWord((BYTE*)(*Opcode+pos),&wOp,&wMem);
					sprintf(menemonic,"%04X%08X",wOp,dwOp);
					strcat((*Disasm)->Opcode,menemonic);
					sprintf(instr,"%08X",dwMem);
					sprintf(menemonic,"%s ptr %s:[%s]",RSize,segs[SEG],instr);					
					(*Disasm)->OpcodeSize=6;
					(*(*index))+=5;
				}
				else{
					...
					sprintf(menemonic,"%04X",wOp);
					strcat((*Disasm)->Opcode,menemonic);
					sprintf(menemonic,"%s ptr %s:[%s]",RSize,segs[SEG],regs[ADDRM][reg1]);
			...
			case 01: // 1 btye extention to regMem
			{
				...
				if(FOpcode>0x7F) // check for signed numbers
				{
					sprintf(Aritmathic,"%s",Scale[0]); // '-' aritmathic
					FOpcode = 0x100-FOpcode; // -XX
				}
				...
				sprintf(menemonic,"%s ptr %s:[%s%s%02Xh]",RSize,segs[SEG],regs[ADDRM][reg1],Aritmathic,FOpcode);
			...
			case 02: // 4 btye extention to regMem
			{
				...
				sprintf(instr,"%08X",dwMem);
				sprintf(menemonic,"%s ptr %s:[%s+%s]",RSize,segs[SEG],regs[ADDRM][reg1],instr);
		...
		switch(Bit_d)
		{
			case 0: // (->) Direction
			{
				...
				switch(Op)// Check for all Cases Availble
				{
                    ...
                    case 0x6B:
                    {
                        ...
                        strcpy(instruction,"imul");                        						
                        sprintf(tempMeme,"%s %s,%s,%s%02Xh",instruction,regs[RM][reg2],menemonic,Aritmathic,FOpcode);
                    ...
                    case 0x81: case 0xC7: case 0x69:// Opcode 0x81/0xC7/0x69
					{
                        ...
                        if(Op==0xC7)
                        {
                            ...
                            */
                            ...
                            {
                                sprintf(instruction,"imul %s,",regs[RM][reg2]);
                            }
                            else
                                sprintf(instruction,"%s",Instructions[REG]);
                        }
                        
                        sprintf(tempMeme,"%s %s,%s",instruction,menemonic,temp);
                        if(PrefixReg==0)
                        {                        
                            (*(*index))+=4;
                            (*Disasm)->OpcodeSize+=4;
                    ...
                    case 0x80:case 0x82: case 0x83: case 0xC6:
					{
                        ...
                        sprintf(tempMeme,"%s %s,%s%02Xh",instruction,menemonic,Aritmathic,FOpcode);
                    ...
                    { 
                        if(REG>5)
                            strcat((*Disasm)->Remarks,";Invalid Segment Usage");

                        sprintf(tempMeme,"%s %s,%s",instruction,menemonic,segs[REG]);
                    }
                    break;
                    ...
                    case 0xD0: case 0xD1:
                    {
                        ...
                        sprintf(tempMeme,"%s %s,1",instruction,menemonic);
                    }
                    break;
                    ...
                    {
                            strcpy(instruction,ArtimaticInstructions[REG]);
                            sprintf(tempMeme,"%s %s,cl",instruction,menemonic);
                    }
                    break;

                    case 0xD8: case 0xDC:// FPU Instruction (unsigned instructions)
                    {
                        sprintf(tempMeme,"%s %s",FpuInstructions[REG],menemonic);
                    }
                    break;
                    ...
                    case 0xD9: // FPU Instructions Set2 (UnSigned)
                    {
                        ...
                        sprintf(tempMeme,"%s %s",FpuInstructionsSet2[REG],menemonic);
                    }
                    break;

                    case 0xDA: case 0xDE:// FPU Instructions (Signed)
                    {
                        sprintf(tempMeme,"%s %s",FpuInstructionsSigned[REG],menemonic);
                    }
                    break;
                    ...
                    case 0xDB: // FPU Instructions Set2 (Signed)
                    {
                        ...
                        sprintf(tempMeme,"%s %s",FpuInstructionsSet2Signed[REG],menemonic);
                    }
                    break;
                    ...
                    case 0xDD: // FPU Instructions Set3
                    {
                        ...
                        sprintf(tempMeme,"%s %s",FpuInstructionsSet3[REG],menemonic);
                    }
                    break;
                    ...
                    case 0xDF: // Extended FPU Instructions Set2 (Signed)
                    {
                        ...
                        sprintf(tempMeme,"%s %s",FpuInstructionsSet2Signed_EX[REG],menemonic);
                    }
                    break;
                    ...
                    case 0xF6: // MIXED Instructions (MUL,DIV,NOT...)
                    {
                       ...
                       {
                           sprintf(tempMeme,"%s %s,%s%02X",instruction,menemonic,Aritmathic,FOpcode);
                           (*(*index))++;
                           (*Disasm)->OpcodeSize++;
                       }
                       else // NOT/NEG/MUL/IMUL/DIV/IDIV instruction must not have operands
                           sprintf(tempMeme,"%s %s",instruction,menemonic);                        
                    ...
                    case 0xFE: // MIX Instructions (INC,DEC,INVALID,INVALID,INVALID...)
                    {                                                
                        sprintf(tempMeme,"%s %s",InstructionsSet3[REG],menemonic);
                    ...
                    case 0xFF:// MIX Instructions (INC,DEC,CALL,PUSH,JMP,FAR JMP,FAR CALL,INVALID)
                    {
                        sprintf(tempMeme,"%s %s",InstructionsSet4[REG],menemonic);
					...
					default:
					{
					   sprintf(tempMeme,"%s %s,%s",instruction,menemonic,regs[RM][REG]);					   
					}
					break;
				}
			...
			case 1: // (<-) Direction of decoding
			{
                switch(Op)
                {
                    case 0x8E: // Segments in Destination Register
                    {
                        sprintf(tempMeme,"%s %s,%s",instruction,segs[REG],menemonic);
                    }
                    break;
                    ...
                    case 0xC0:case 0xC1:
                    {
                        ...
                        sprintf(tempMeme,"%s %s,%02X",ArtimaticInstructions[REG],menemonic,FOpcode);
						//sprintf(tempMeme,"%s %s,cl",ArtimaticInstructions[REG],menemonic,FOpcode);
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*(*index))++;
                        (*Disasm)->OpcodeSize++;
                    }
                    ...
                    {
                        strcpy(instruction,"les");
                        sprintf(tempMeme,"%s %s,%s",instruction,regs[RM][REG],menemonic);
                    }
                    break;
	...
	else if(SIB==SIB_EX) // Found SIB, lets strip the extensions
	{
        ...
        */
		...
		switch(SCALE)
		{
			case 0:sprintf(Aritmathic,"%s",Scale[1]);break; // +
			case 1:sprintf(Aritmathic,"%s",Scale[2]);break; // *2+
			case 2:sprintf(Aritmathic,"%s",Scale[3]);break; // *4+
			case 3:sprintf(Aritmathic,"%s",Scale[4]);break; // *8+
		}
		...
		switch(Extension) // +/+00/+00000000
		{
			case 00: // No extension of bytes
			{
                if(reg1==REG_ESP && reg2!=REG_EBP)
                {
                    ...
                    sprintf(menemonic,"%02X%04X",Op,wOp);
                    strcat((*Disasm)->Opcode,menemonic);
                    sprintf(menemonic,"%s ptr %s:[%s]",RSize,segs[SEG],regs[ADDRM][reg2]);
                    (*(*index))+=2; //2 byte read				
					(*Disasm)->OpcodeSize=3; // total used opcodes
                }
				else if(reg2!=REG_EBP) // No EBP in RegMem
				{
					...
					sprintf(menemonic,"%02X%04X",Op,wOp);
					strcat((*Disasm)->Opcode,menemonic);
					sprintf(menemonic,"%s ptr %s:[%s%s%s]",RSize,segs[SEG],regs[ADDRM][reg1],Aritmathic,regs[ADDRM][reg2]);
					(*(*index))+=2; //2 byte read				
					(*Disasm)->OpcodeSize=3; // total used opcodes
				}
				else if(reg2==REG_EBP) // Replace EBP with Dword Number
				{
                    ...
                    SwapDword((BYTE*)(*Opcode+pos+3),&dwOp,&dwMem);
                    SwapWord((BYTE*)(*Opcode+pos+1),&wOp,&wMem);
					sprintf(menemonic,"%02X %04X %08X",Op,wOp,dwOp);
                    ...
                    if(reg1==REG_ESP) // Must Not Be ESP (Index)
                    {
                        ...
                        sprintf(temp,regs[ADDRM][reg1]);

					sprintf(menemonic,"%s ptr %s:[%s%s%08X]",
			...
			{				
				...
				if(FOpcode>0x7F) // check for signed numbers!!
				{
					sprintf(tempAritmathic,"%s",Scale[0]); // '-' aritmathic
					FOpcode = 0x100-FOpcode; // -XX
				}
                ...
                {                
					...
					//sprintf(tempMeme,"%s ptr %s:[%s%s%s%s%02X]",RSize,segs[SEG],regs[ADDRM][reg2],tempAritmathic,regs[ADDRM][reg1],tempAritmathic,FOpcode); ; OLD
                    sprintf(tempMeme,"%s ptr %s:[%s%s%02Xh]",RSize,segs[SEG],regs[ADDRM][reg1],tempAritmathic,FOpcode);
                }
				else
				    sprintf(tempMeme,"%s ptr %s:[%s%s%s%s%02Xh]",RSize,segs[SEG],regs[ADDRM][reg1],Aritmathic,regs[ADDRM][reg2],tempAritmathic,FOpcode);
			...
			case 02:// Dword extension
			{
                ...
                if(reg1!=REG_ESP) 
                {
                    ...
                    sprintf(tempMeme,"%s ptr %s:[%s%s%s%s%08Xh]",
                ...
                {// ESP Must not be as Index, Code = 100b
                    ...
                    sprintf(tempMeme,"%s ptr %s:[%s%s%08Xh]",
                        RSize,  // size of register
                        segs[SEG], // segment
                        regs[ADDRM][reg2],
                        tempAritmathic,
                    ...
                    case 0xC0:case 0xC1:
                    {
                        switch(Extension)
                        {
                            ...
                            case 1:
                            {
                                FOpcode=(BYTE)(*(*Opcode+pos+4));
                                sprintf(tempMeme,"%s %s,%02X",ArtimaticInstructions[REG],menemonic,FOpcode);                                
                                //sprintf(menemonic," %02X",(BYTE)(*(*Opcode+pos+4)));
                                sprintf(menemonic," %02X",FOpcode);
                                strcat((*Disasm)->Opcode,menemonic);
                                (*(*index))++;
                                (*Disasm)->OpcodeSize++;
...
void GetInstruction(BYTE Opcode,char *menemonic)
{
	...
	// הפונקציה תמצא איזו פקודה שייכת
	// לכל בייט.
	// ישנם 9 פקודות שונות עם  סוגים שונים של קודים
...
int GetNewInstruction(BYTE Op,char *ASM,bool RegPrefix)
{
    ...
    switch(Op)
    {
        ...
        case 0xCC: case 0xCD: case 0xCE: case 0xCF:
        {
            ...
            sprintf(Inst,"bswap %s",regs[RM][Op&7]);
        }
        break;        
        ...
        {
            sprintf(Inst,"%s ",JumpTable[Op&0x0F]);
            Found=2;
        }
        break;
    ...
    */
    ...
    switch(Op)
    {
        case 0x00: 
        {
            ...
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(assembly,"%s %s",NewSet[REG],regs[RM][reg2]);
            sprintf(temp,"%04X",wOp);
            if(REG>5)
                strcat((*Disasm)->Remarks,"Invalid Instruction");
        }
        ...
        case 0x01: 
        {
            ...
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(assembly,"%s %s",NewSet2[REG],regs[RM][reg2]);
            sprintf(temp,"%04X",wOp);
            if(REG==5)
                strcat((*Disasm)->Remarks,"Invalid Instruction");
        }
        ...
        case 0x02: // LAR
        {
            sprintf(assembly,"lar %s, %s",regs[RM][reg1],regs[RM][reg2]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x03: // LSL
        {
          sprintf(assembly,"lsl %s, %s",regs[RM][reg1],regs[RM][reg2]);
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x10: // MOVUPS
        {
           if(RepPrefix)
           {
               sprintf(assembly,"movss %s, %s",MMXRegs[reg1],MMXRegs[reg2]);
               strcpy((*Disasm)->Assembly,"");
               m_OpcodeSize++;
           }
           else
               sprintf(assembly,"movups %s, %s",MMXRegs[reg1],MMXRegs[reg2]);
        ...
        case 0x11: // MOVUPS
        {
           if(RepPrefix)
           {
                sprintf(assembly,"movss %s, %s",MMXRegs[reg2],MMXRegs[reg1]);
                strcpy((*Disasm)->Assembly,"");
                m_OpcodeSize++;
           }
           else
               sprintf(assembly,"movups %s, %s",MMXRegs[reg2],MMXRegs[reg1]);
        ...
        case 0x12: // MOVHLPS
        {
            sprintf(assembly,"movhlps %s, %s",MMXRegs[reg1],MMXRegs[reg2]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x13: // MOVLPS
        {
            sprintf(assembly,"movlps %s, %s",MMXRegs[reg2],MMXRegs[reg1]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x14: // UNPCKLPS
        {
           sprintf(assembly,"unpcklps %s, %s",MMXRegs[reg1],MMXRegs[reg2]);
           SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
           sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x15: // UNPCKHPS
        {
          sprintf(assembly,"unpckhps %s, %s",MMXRegs[reg1],MMXRegs[reg2]);
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x16: // MOVLHPS
        {
          sprintf(assembly,"movhlhps %s, %s",MMXRegs[reg1],MMXRegs[reg2]);
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x17: // MOVHPS
        {
           sprintf(assembly,"movhps %s, %s",MMXRegs[reg2],MMXRegs[reg1]);
           SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
           sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x18:
        {
           sprintf(assembly,"%s, %s",NewSet3[REG],regs[RM][reg2]);
           if(REG>3)
               strcat((*Disasm)->Remarks,"Invalid Instruction");
        ...
        case 0x28: // MOVAPS
        {
           sprintf(assembly,"movaps %s, %s",MMXRegs[reg1],MMXRegs[reg2]);
           SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
           sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x29: // MOVAPS
        {
          sprintf(assembly,"movaps %s, %s",MMXRegs[reg2],MMXRegs[reg1]);
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x2A: // CVTPI2PS
        {
          if(RepPrefix==1)
          {
              sprintf(assembly,"cvtsi2ss %s, %s",MMXRegs[reg1],regs[RM][reg2]);
              strcpy((*Disasm)->Assembly,"");
              m_OpcodeSize++;
          }
          else
              sprintf(assembly,"%s %s, %s",NewSet4[(Op&0x0F)-0x08],MMXRegs[reg1],Regs3DNow[reg2]);
        ...
        case 0x2C: case 0x2D: // CVTTPS2PI, CVTPS2PI
        {
            if(RepPrefix==1)
            {
                ...
                sprintf(assembly,"%s %s, %s",temp,regs[RM][reg1],MMXRegs[reg2]);
                strcpy((*Disasm)->Assembly,"");
                m_OpcodeSize++;
            }
            else
                sprintf(assembly,"%s %s, %s",NewSet4[(Op&0x0F)-0x08],Regs3DNow[reg1],MMXRegs[reg2]);
        ...
        {
            sprintf(assembly,"%s %s, %s",NewSet4[(Op&0x0F)-0x08],MMXRegs[reg1],MMXRegs[reg2]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        {
          sprintf(assembly,"%s %s,%s",NewSet5[Op&0x0F],regs[RM][reg1],regs[RM][reg2]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x58:case 0x59:case 0x5C:case 0x5D:case 0x5E:case 0x5F:
        {
            ...
            {
                sprintf(assembly,"%s %s,%s",NewSet6Ex[Op&0x0F],MMXRegs[reg1],MMXRegs[reg2]);
                strcpy((*Disasm)->Assembly,"");
                m_OpcodeSize++;
            }
            else
                sprintf(assembly,"%s %s,%s",NewSet6[Op&0x0F],MMXRegs[reg1],MMXRegs[reg2]); 
        ...
        {
          sprintf(assembly,"%s %s,%s",NewSet7[Op&0x0F],Regs3DNow[reg1],Regs3DNow[reg2]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x70:
        {
            sprintf(assembly,"%s %s,%s,%02X",NewSet8[Op&0x0F],Regs3DNow[reg1],Regs3DNow[reg2],(BYTE)(*(*Opcode+Pos+2))); 
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X %02X",wOp,(BYTE)(*(*Opcode+Pos+2)));
            (*(*index))++;
            m_OpcodeSize++;
        }
        break;
        ...
        {
          sprintf(assembly,"%s %s,%s",NewSet8[Op&0x0F],Regs3DNow[reg1],Regs3DNow[reg2]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x7E:
        {
          sprintf(assembly,"%s %s,%s",NewSet7[Op&0x0F],regs[RM][reg2],Regs3DNow[reg1]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x7F:
        {
          sprintf(assembly,"%s %s,%s",NewSet7[Op&0x0F],Regs3DNow[reg2],Regs3DNow[reg1]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        {
          RM=REG8; // FORCE 8BIT
          sprintf(assembly,"%s %s",NewSet9[Op&0x0F],regs[RM][reg2]);
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break; // MIX
        ...
        case 0xA3: case 0xAB:
        {
          sprintf(assembly,"%s %s,%s",NewSet10[Op&0x0F],regs[RM][reg2],regs[RM][reg1]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0xA4: case 0xAC:
        {
            sprintf(assembly,"%s %s,%s,%02X",NewSet10[Op&0x0F],regs[RM][reg2],regs[RM][reg1],(BYTE)(*(*Opcode+Pos+2))); 
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X %02X",wOp,(BYTE)(*(*Opcode+Pos+2)));
            (*(*index))++;
            m_OpcodeSize++;
        ...
        case 0xA5: case 0xAD:
        {
          sprintf(assembly,"%s %s,%s,cl",NewSet10[Op&0x0F],regs[RM][reg2],regs[RM][reg1]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0xAE:
        {            
            sprintf(temp,"%02X%02X",(BYTE)(*(*Opcode+Pos)),(BYTE)(*(*Opcode+Pos+1))); 
            ...
            if(REG>3)// Check for Invalid
            {
                ...
                switch(m_Opcode) // Lone Instructions
                {
                    ...
                    default: strcat((*Disasm)->Remarks,"Invalid Instruction"); break;
                }                
            }
            else
                sprintf(assembly,"%s %s",NewSet10Ex[REG],regs[RM][reg2]);
        }
        break;

        case 0xAF:
        {
          sprintf(assembly,"%s %s,%s",NewSet10[Op&0x0F],regs[RM][reg1],regs[RM][reg2]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0xB0: case 0xB1: case 0xB3: case 0xBB:
        {
          ...
          sprintf(assembly,"%s %s,%s",NewSet11[Op&0x0F],regs[RM][reg2],regs[RM][reg1]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0xBD:case 0xBE:case 0xBF:
        {
          ...
          sprintf(assembly,"%s %s,%s",NewSet11[Op&0x0F],regs[RM][reg1],regs[RM2][reg2]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0xC0: case 0xC1:
        {
          ...
          sprintf(assembly,"xadd %s,%s",regs[RM][reg2],regs[RM][reg1]); 
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0xC2:
        {
            if((BYTE)(*(*Opcode+Pos+2))<8) // Instructions here
            {
                if(RepPrefix==1)
                {
                    sprintf(assembly,"%s %s,%s",NewSet12Ex[(BYTE)(*(*Opcode+Pos+2))],MMXRegs[reg1],MMXRegs[reg2]);
                    strcpy((*Disasm)->Assembly,"");
                    m_OpcodeSize++;
                }
                else
                    sprintf(assembly,"%s %s,%s",NewSet12[(BYTE)(*(*Opcode+Pos+2))],MMXRegs[reg1],MMXRegs[reg2]);
            }
            else
            {
                if(RepPrefix==1)
                {
                    sprintf(assembly,"cmpss %s,%s,%02X",MMXRegs[reg1],MMXRegs[reg2],(BYTE)(*(*Opcode+Pos+2)));
                    strcpy((*Disasm)->Assembly,"");
                    m_OpcodeSize++;
                }
                else
                    sprintf(assembly,"cmpps %s,%s,%02X",MMXRegs[reg1],MMXRegs[reg2],(BYTE)(*(*Opcode+Pos+2)));
            }
        ...
        {
            RM=REG16;
            sprintf(assembly,"pinsrw %s,%s,%02X",Regs3DNow[reg1],regs[RM][reg2],(BYTE)(*(*Opcode+Pos+2)));
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X %02X",wOp,(BYTE)(*(*Opcode+Pos+2)));
            (*(*index))++;
            m_OpcodeSize++;
        ...
        case 0xC5:
        {
            sprintf(assembly,"pextrw %s,%s,%02X",regs[RM][reg1],Regs3DNow[reg2],(BYTE)(*(*Opcode+Pos+2)));
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X %02X",wOp,(BYTE)(*(*Opcode+Pos+2)));
            (*(*index))++;
            m_OpcodeSize++;
        ...
        case 0xC6:
        {
            sprintf(assembly,"pextrw %s,%s,%02X",MMXRegs[reg1],MMXRegs[reg2],(BYTE)(*(*Opcode+Pos+2)));
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X %02X",wOp,(BYTE)(*(*Opcode+Pos+2)));
            (*(*index))++;
            m_OpcodeSize++;
        ...
        case 0xD7: 
        {
            sprintf(assembly,"%s %s,%s",NewSet13[Op&0x0F],regs[RM][reg1],Regs3DNow[reg2]); 
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        {
            sprintf(assembly,"%s %s,%s",NewSet13[Op&0x0F],Regs3DNow[reg1],Regs3DNow[reg2]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0xEE:case 0xEF:
        {
            sprintf(assembly,"%s %s,%s",NewSet14[Op&0x0F],Regs3DNow[reg1],Regs3DNow[reg2]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0xE7:
        {
            sprintf(assembly,"%s %s,%s",NewSet14[Op&0x0F],regs[RM][reg2],Regs3DNow[reg1]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x0D:
        {
            ...
            if((NextByte)<=0xCF)
                sprintf(assembly,"%s %s",temp,regs[RM][reg2]);
            else
            {
                strcpy(assembly,"???");
                strcat((*Disasm)->Remarks,"Invalid Instruction");
        ...
        case 0x20:
        {
            sprintf(assembly,"mov %s,%s",regs[RM][reg2],ControlRegs[reg1]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x21:
        {
          sprintf(assembly,"mov %s,%s",regs[RM][reg2],DebugRegs[reg1]);
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x22:
        {
          sprintf(assembly,"mov %s,%s",ControlRegs[reg1],regs[RM][reg2]);
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x23:
        {
          sprintf(assembly,"mov %s,%s",DebugRegs[reg1],regs[RM][reg2]);
          SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
          sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x50:
        {
            sprintf(assembly,"movmskps %s,%s",regs[RM][reg1],MMXRegs[reg2]);
            SwapWord((BYTE*)(*Opcode+Pos),&wOp,&wMem);
            sprintf(temp,"%04X",wOp);
        }
        break;
        ...
        case 0x71:
        {
            ...
            {            
                ...
                sprintf(assembly,"%s %s,%02X",temp,Regs3DNow[reg2],(BYTE)(*(*Opcode+Pos+2)));
            }
            else
            {
                strcpy(assembly,"???");
        ...
        case 0xBA:
        {
            ...
            {            
                ...
                sprintf(assembly,"%s %s,%02X",temp,regs[RM][reg2],(BYTE)(*(*Opcode+Pos+2)));
            }
            else
            {
                strcpy(assembly,"???");
        ...
        switch(Bit_d)
        {
            case 0: // direction (->)
			{
                switch(Op)
                {
                    case 0x00:
                    {
                        sprintf(temp,"%s %s",NewSet[REG],tempMeme);
                    ...
                    case 0x01:
                    {
                        sprintf(temp,"%s %s",NewSet2[REG],tempMeme);
                    ...
                    case 0x11: 
                        {
                            if(RepPrefix==1)
                            {
                                sprintf(temp,"movss %s,%s",tempMeme,MMXRegs[REG]);
                                strcpy((*Disasm)->Assembly,"");
                                (*Disasm)->OpcodeSize++;
                            }
                            else
                                sprintf(temp,"movups %s,%s",tempMeme,MMXRegs[REG]); 
                        }
                        break; // MOVUPS
                    case 0x13: sprintf(temp,"movlps %s,%s",tempMeme,MMXRegs[REG]); break; // MOVLPS
                    case 0x17: sprintf(temp,"movhps %s,%s",tempMeme,MMXRegs[REG]); break; // MOVHPS
                    case 0x29: sprintf(temp,"movaps %s,%s",tempMeme,MMXRegs[REG]); break; // MOVAPS
                    case 0x7E: case 0x7F:sprintf(temp,"%s %s,%s",NewSet7[Op&0x0F],tempMeme,Regs3DNow[REG]); break; // MOVD/MOVQ
                    case 0xA3: case 0xAB:
                    {
                      sprintf(temp,"%s %s,%s",NewSet10[Op&0x0F],tempMeme,regs[RM][REG]); 
                    }
                    break;

                    case 0xA4:case 0xAC:
                    {
                       sprintf(temp,"%s %s,%s,%02X",NewSet10[Op&0x0F],tempMeme,regs[RM][REG],FOpcode);
                       sprintf(menemonic," %02X",FOpcode);
                       strcat((*Disasm)->Opcode,menemonic);
                       (*Disasm)->OpcodeSize++;
                       (*(*index))++;
                    ...
                    case 0xA5: case 0xAD:
                    {
                      sprintf(temp,"%s %s,%s,cl",NewSet10[Op&0x0F],tempMeme,regs[RM][REG]); 
                    }
                    break;
                    ...
                    case 0xB0: case 0xB1: case 0xB3: case 0xBB:
                    {
                      ...
                      sprintf(temp,"%s %s,%s",NewSet11[Op&0x0F],tempMeme,regs[RM][REG]);
                    }
                    break;
                    ...
                    { 
                        RM=REG8; 
                        sprintf(temp,"xadd %s,%s",tempMeme,regs[RM][REG]); 
                    } 
                    break; // XADD

                    case 0xC1:sprintf(temp,"xadd %s,%s",tempMeme,regs[RM][REG]); break;
                    case 0xE7:sprintf(temp,"%s %s,%s",NewSet14[Op&0x07],tempMeme,Regs3DNow[REG]); break;
            ...
            case 1: // direction (<-)
            {
                switch(Op)
                {                
                    case 0x02: sprintf(temp,"lar %s,%s",regs[RM][REG],tempMeme);     break; // LAR
                    case 0x03: sprintf(temp,"lsl %s,%s",regs[RM][REG],tempMeme);     break; // LAR
                    case 0x10: 
                        {
                          if(RepPrefix==1)
                          {
                            sprintf(temp,"movss %s,%s",MMXRegs[REG],tempMeme);
                            strcpy((*Disasm)->Assembly,"");
                            (*Disasm)->OpcodeSize++;
                          }
                          else
                            sprintf(temp,"movups %s,%s",MMXRegs[REG],tempMeme);
                        }
                        break; // MOVUPS
                    
                    case 0x12: sprintf(temp,"movlps %s,%s",MMXRegs[REG],tempMeme);   break; // MOVLPS
                    case 0x14: sprintf(temp,"unpcklps %s,%s",MMXRegs[REG],tempMeme); break; // UNPCKLPS
                    case 0x15: sprintf(temp,"unpckhps %s,%s",MMXRegs[REG],tempMeme); break; // UNPCKHPS
                    case 0x16: sprintf(temp,"movhps %s,%s",MMXRegs[REG],tempMeme);   break; // MOVHPS
                    
                    case 0x18:
                    {
                      sprintf(temp,"%s,%s",NewSet3[REG],tempMeme);
                      if(REG>3)
                         strcat((*Disasm)->Remarks,"Invalid instruction");
                    } 
                    break; // MIX
                    
                    case 0x28: sprintf(temp,"movaps %s,%s",MMXRegs[REG],tempMeme);   break; // MOVAPS
                    
                    case 0x2A: case 0x2C: case 0x2D:case 0x2E: case 0x2F: 
                    {
                      BYTE R=((Op&0x0F)-0x08);
                      if(R==4 || R==5)
                          sprintf(instr,"%s",Regs3DNow[REG]); // 3DNow! Regs
                      else
                          sprintf(instr,"%s",MMXRegs[REG]); // MMX Regs
                      ...
                      if(RepPrefix==1)
                      {
                          if(Op==0x2A || Op==0x2C || Op==0x2D)
                          {
                              ...
                              strcpy((*Disasm)->Assembly,"");
                              (*Disasm)->OpcodeSize++;
                              sprintf(temp,"%s %s,%s",menemonic,instr,tempMeme);
                          }
                      }
                      else
                        sprintf(temp,"%s %s,%s",NewSet4[(Op&0x0F)-0x08],instr,tempMeme); 
                    }
                    break; // MIX
                    ...
                    case 0x48:case 0x49:case 0x4A:case 0x4B:case 0x4C:case 0x4D:case 0x4E:case 0x4F:
                    {
                       sprintf(temp,"%s %s,%s",NewSet5[Op&0x0F],regs[RM][REG],tempMeme); 
                    }
                    break; // MIX
                    ...
                    case 0x58:case 0x59:case 0x5C:case 0x5D:case 0x5E:case 0x5F:
                    {
                        if(RepPrefix==1)
                        {
                            sprintf(temp,"%s %s,%s",NewSet6Ex[Op&0x0F],MMXRegs[reg2],tempMeme);
                            strcpy((*Disasm)->Assembly,"");
                            (*Disasm)->OpcodeSize++;
                        }
                        else
                            sprintf(temp,"%s %s,%s",NewSet6[Op&0x0F],MMXRegs[REG],tempMeme); 
                    }
                    break; // MIX
                    ...
                    case 0x68:case 0x69:case 0x6A:case 0x6B:case 0x6E:case 0x6F:
                    {
                       sprintf(temp,"%s %s,%s",NewSet7[Op&0x0F],Regs3DNow[REG],tempMeme); 
                    }
                    break;

                    case 0x70:
                    {                       
                        sprintf(temp,"%s %s,%s,%02X",NewSet8[Op&0x0F],Regs3DNow[reg2],tempMeme,FOpcode);
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0x74:case 0x75:case 0x76: // MIX
                    {
                       sprintf(temp,"%s %s,%s",NewSet8[Op&0x0F],Regs3DNow[REG],tempMeme); 
                    }
                    break;
                    ...
                    case 0x98:case 0x99:case 0x9A:case 0x9B:case 0x9C:case 0x9D:case 0x9E:case 0x9F:
                    {
                      sprintf(temp,"%s %s",NewSet9[Op&0x0F],tempMeme);
                    }
                    break; // MIX

                    case 0xAE: // FXSAVE
                    {
                        sprintf(temp,"%s %s",NewSet10Ex[REG],tempMeme);
                        if(REG>3)// Check for Invalid
                            strcat((*Disasm)->Remarks,"Invalid Instruction");
                    }
                    break;
                    case 0xAF:sprintf(temp,"%s %s,%s",NewSet10[Op&0x0F],regs[RM][REG],tempMeme);break;
                    
                    case 0xB2:case 0xB4:case 0xB5:
                    case 0xB6:case 0xB7:case 0xBC: 
                    case 0xBD:case 0xBE:case 0xBF:
                    {
                      sprintf(temp,"%s %s,%s",NewSet11[Op&0x0F],regs[RM][REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xC2:
                    {
                      if(FOpcode<8) // Instructions here
                      {
                          if(RepPrefix==1) // Rep Prefix is being used
                          {
                              sprintf(temp,"%s %s,%s",NewSet12Ex[FOpcode],MMXRegs[REG],tempMeme);
                              strcpy((*Disasm)->Assembly,"");
                              (*Disasm)->OpcodeSize++;
                          }
                          else
                              sprintf(temp,"%s %s,%s",NewSet12[FOpcode],MMXRegs[REG],tempMeme);
                      }
                      else
                      {
                          if(RepPrefix==1) // Rep Prefix is being used
                          {
                              sprintf(temp,"cmpss %s,%s,%02X",MMXRegs[REG],tempMeme,FOpcode);
                              strcpy((*Disasm)->Assembly,"");
                              (*Disasm)->OpcodeSize++;
                          }
                          else
                              sprintf(temp,"cmpps %s,%s,%02X",MMXRegs[REG],tempMeme,FOpcode);
                      }
                    ...
                    case 0xC4:
                    {
                        sprintf(temp,"pinsrw %s,%s,%02X",Regs3DNow[REG],tempMeme,FOpcode);                        
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0xC5:
                    {
                        sprintf(temp,"pextrw %s,%s,%02X",regs[RM][REG],tempMeme,FOpcode);                        
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0xC6:
                    {
                        sprintf(temp,"shufps %s,%s,%02X",MMXRegs[REG],tempMeme,FOpcode);                        
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0xD7:sprintf(temp,"%s %s,%s",NewSet13[Op&0x0F],regs[RM][REG],tempMeme);break;
                    case 0xD1:case 0xD2:case 0xD3:case 0xD5:case 0xD8:case 0xDF:
                    case 0xD9:case 0xDA:case 0xDB:case 0xDC:case 0xDD:case 0xDE:                    
                    {
                        sprintf(temp,"%s %s,%s",NewSet13[Op&0x0F],Regs3DNow[REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xEE:case 0xEF:
                    {
                      sprintf(temp,"%s %s,%s",NewSet14[Op&0x0F],Regs3DNow[REG],tempMeme);
                    }
                    break;
                    ...
                    case 0xFD:case 0xFE:
                    {
                      sprintf(temp,"%s %s,%s",NewSet15[Op&0x0F],Regs3DNow[REG],tempMeme);
                    }
                    break;
                }
    ...
    if(SIB!=SIB_EX) // NO SIB extension (i.e: 0x0001 = add byte ptr [ecx], al)
    {
        ...
        switch(Extension) // Check what extension we have (None/Byte/Dword)
        {
            ...
            case 01: // 1 btye extention to regMem
            {
                FOpcode=(BYTE)(*(*Opcode+pos+2));
                SwapWord((BYTE*)(*Opcode+pos),&wOp,&wMem);
                sprintf(menemonic,"%04X%02X",wOp,FOpcode);
                strcat((*Disasm)->Opcode,menemonic);
                ...
                sprintf(menemonic,"%s ptr %s:[%s%s%02X]",RSize,segs[SEG],regs[ADDRM][reg1],Aritmathic,FOpcode);
                (*(*index))+=2; // x + 1 byte(s) read
                (*Disasm)->OpcodeSize=3; // total used opcodes
                FOpcode=(BYTE)(*(*Opcode+pos+3));
            }
        ...
        {            
            case 0: // direction (->)
            {
                switch(Op) // Check for all Cases Availble            
                {
                    ...
                    {
                        sprintf(tempMeme,"%s %s",NewSet[REG],menemonic);
                    ...
                    {
                        sprintf(tempMeme,"%s %s",NewSet2[REG],menemonic);
                    ...
                    case 0x11: 
                        {
                            if(RepPrefix==1)
                            {
                                sprintf(tempMeme,"movss %s,%s",menemonic,MMXRegs[reg2]);
                                strcpy((*Disasm)->Assembly,"");
                                (*Disasm)->OpcodeSize++;
                            }
                            else
                                sprintf(tempMeme,"movups %s,%s",menemonic,MMXRegs[reg2]); 
                        }
                        break; // MOVUPS
                    case 0x13: sprintf(tempMeme,"movlps %s,%s",menemonic,MMXRegs[reg2]); break; // MOVLPS
                    case 0x17: sprintf(tempMeme,"movhps %s,%s",menemonic,MMXRegs[reg2]); break; // MOVHPS
                    case 0x29: sprintf(tempMeme,"movaps %s,%s",menemonic,MMXRegs[reg2]); break; // MOVAPS
                    case 0x7E: case 0x7F:sprintf(tempMeme,"%s %s,%s",NewSet7[Op&0x0F],menemonic,Regs3DNow[reg2]); break; // MOVD/MOVQ
                    case 0xA3: case 0xAB:
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet10[Op&0x0F],menemonic,regs[RM][reg2]); 
                    }
                    break;
                    case 0xA4:case 0xAC:
                    {
                       sprintf(tempMeme,"%s %s,%s,%02X",NewSet10[Op&0x0F],menemonic,regs[RM][reg2],FOpcode);
                       sprintf(menemonic," %02X",FOpcode);
                       strcat((*Disasm)->Opcode,menemonic);
                       (*Disasm)->OpcodeSize++;
                       (*(*index))++;
                    ...
                    break;
                    case 0xA5: case 0xAD:sprintf(tempMeme,"%s %s,%s,cl",NewSet10[Op&0x0F],menemonic,regs[RM][reg2]); break;
                    ...
                    case 0xB0: case 0xB1:  case 0xB3: case 0xBB:
                    {
                        ...
                        sprintf(tempMeme,"%s %s,%s",NewSet11[Op&0x0F],menemonic,regs[RM][reg2]);
                    }
                    break;
                    case 0xC0: RM=REG8; sprintf(tempMeme,"xadd %s,%s",menemonic,regs[RM][reg2]);break;
                    case 0xC1: sprintf(tempMeme,"xadd %s,%s",menemonic,regs[RM][reg2]); break;
                    case 0xE7: sprintf(tempMeme,"%s %s,%s",NewSet14[Op&0x07],menemonic,Regs3DNow[reg2]); break;
                }

                strcat((*Disasm)->Assembly,tempMeme); // copy the decoded assembly
            }
            break;
            ...
            case 1: // direction (<-)
            {
                switch(Op)
                {                 
                    case 0x02: sprintf(tempMeme,"lar %s,%s",regs[RM][reg2],menemonic);     break; // LAR
                    case 0x03: sprintf(tempMeme,"lsl %s,%s",regs[RM][reg2],menemonic);     break; // LSL
					...
					case 0x0D:
					{
						  ...
						  )
						{
							if((FOpcode&0x0F)<=0x07)
								sprintf(tempMeme,"prefetcht %s",menemonic);
							else
								sprintf(tempMeme,"prefetchtW %s",menemonic);
						}
						else{
								strcpy(tempMeme,"???");
						}
					...
					case 0x10: 
                    {
						if(RepPrefix==1) // rep prefix is being used
						{
							strcpy((*Disasm)->Assembly,"");
							(*Disasm)->OpcodeSize++;
							sprintf(tempMeme,"movss %s,%s",MMXRegs[reg2],menemonic);
						}
						else 
							sprintf(tempMeme,"movups %s,%s",MMXRegs[reg2],menemonic); 
                    }
                    break; // MOVUPS
                    case 0x12: sprintf(tempMeme,"movlps %s,%s",MMXRegs[reg2],menemonic);   break; // MOVLPS
                    case 0x14: sprintf(tempMeme,"unpcklps %s,%s",MMXRegs[reg2],menemonic); break; // UNPCKLPS
                    case 0x15: sprintf(tempMeme,"unpckhps %s,%s",MMXRegs[reg2],menemonic); break; // UNPCKHPS
                    case 0x16: sprintf(tempMeme,"movhps %s,%s",MMXRegs[reg2],menemonic);   break; // MOVHPS
                    
                    case 0x18:
                    {
                        sprintf(tempMeme,"%s,%s",NewSet3[REG],menemonic);
                        if(REG>3)
                            strcat((*Disasm)->Remarks,"Invalid instruction");
                    } 
                    break;
                    
                    case 0x28: sprintf(tempMeme,"movaps %s,%s",MMXRegs[reg2],menemonic);   break; // MOVAPS
                    ...
                    case 0x2A: case 0x2C: case 0x2D:case 0x2E: case 0x2F: 
                    {
                       ...
                       if(R==4 || R==5)
                           sprintf(temp,"%s",Regs3DNow[reg2]); // 3DNow! Regs
                       else
                           sprintf(temp,"%s",MMXRegs[reg2]); // MMX Regs
                       ...
                       if(RepPrefix==1)
                       {
                           ...
                           if(Op==0x2A || Op==0x2C || Op==0x2D)
                           {
                               ...
                               strcpy((*Disasm)->Assembly,"");
                               (*Disasm)->OpcodeSize++;
                               sprintf(tempMeme,"%s %s,%s",instruction,temp,menemonic);
                           }
                      }
                      else
                          sprintf(tempMeme,"%s %s,%s",NewSet4[(Op&0x0F)-0x08],temp,menemonic); 
                    }
                    break; // MIX
                    ...
                    case 0x48:case 0x49:case 0x4A:case 0x4B:case 0x4C:case 0x4D:case 0x4E:case 0x4F:
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet5[Op&0x0F],regs[RM][reg2],menemonic); 
                    }
                    break;
                    ...
                    case 0x58:case 0x59:case 0x5C:case 0x5D:case 0x5E:case 0x5F:
                    {
                        if(RepPrefix==1)
                        {
                            sprintf(tempMeme,"%s %s,%s",NewSet6Ex[Op&0x0F],MMXRegs[reg2],menemonic);
                            strcpy((*Disasm)->Assembly,"");
                            (*Disasm)->OpcodeSize++;
                        }
                        else
                            sprintf(tempMeme,"%s %s,%s",NewSet6[Op&0x0F],MMXRegs[reg2],menemonic); 
                    }
                    break;
                    ...
                    case 0x68:case 0x69:case 0x6A:case 0x6B:case 0x6E:case 0x6F:
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet7[Op&0x0F],Regs3DNow[reg2],menemonic); 
                    }
                    break; // MIX

                    case 0x70: // PSHUFW
                    {                       
                       sprintf(tempMeme,"%s %s,%s,%02X",NewSet8[Op&0x0F],Regs3DNow[reg2],menemonic,FOpcode);
                       sprintf(menemonic," %02X",FOpcode);
                       strcat((*Disasm)->Opcode,menemonic);
                       (*Disasm)->OpcodeSize++;
                       (*(*index))++;
                    ...
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet8[Op&0x0F],Regs3DNow[reg2],menemonic); 
                    }
                    break;
                    ...
                    case 0x98:case 0x99:case 0x9A:case 0x9B:case 0x9C:case 0x9D:case 0x9E:case 0x9F:
                    {
                        sprintf(tempMeme,"%s %s",NewSet9[Op&0x0F],menemonic);
                    }
                    break; // MIX

                    case 0xAE:
                    {
                        sprintf(tempMeme,"%s %s",NewSet10Ex[REG],menemonic);
                        if(REG>3)// Check for Invalid
                            strcat((*Disasm)->Remarks,"Invalid Instruction");
                    }
                    break;
                    case 0xAF:sprintf(tempMeme,"%s %s,%s",NewSet10[Op&0x0F],regs[RM][reg2],menemonic);break;

                    case 0xB2:case 0xB4:case 0xB5:
                    case 0xB6:case 0xB7:case 0xBC: 
                    case 0xBD:case 0xBE:case 0xBF:
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet11[Op&0x0F],regs[RM][reg2],menemonic);
                    }
                    break;
                    ...
                    case 0xC2:
                    {
                       if(FOpcode<8) // Instructions here
                       {
                           if(RepPrefix==1) // Rep Prefix is being used
                           {
                               sprintf(tempMeme,"%s %s,%s",NewSet12Ex[FOpcode],MMXRegs[reg2],menemonic);
                               strcpy((*Disasm)->Assembly,"");
                               (*Disasm)->OpcodeSize++;
                           }
                           else
                               sprintf(tempMeme,"%s %s,%s",NewSet12[FOpcode],MMXRegs[reg2],menemonic);
                       }
                       else
                       {
                           if(RepPrefix==1) // Rep Prefix is being used
                           {
                               sprintf(tempMeme,"cmpss %s,%s,%02X",MMXRegs[reg2],menemonic,FOpcode);
                               strcpy((*Disasm)->Assembly,"");
                               (*Disasm)->OpcodeSize++;
                           }
                           else 
                               sprintf(tempMeme,"cmpps %s,%s,%02X",MMXRegs[reg2],menemonic,FOpcode);
                       }
                    ...
                    case 0xC4:
                    {
                        sprintf(tempMeme,"pinsrw %s,%s,%02X",Regs3DNow[reg2],menemonic,FOpcode);                        
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0xC5:
                    {
                        sprintf(tempMeme,"pextrw %s,%s,%02X",regs[RM][reg2],menemonic,FOpcode);                        
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0xC6:
                    {
                        sprintf(tempMeme,"shufps %s,%s,%02X",MMXRegs[reg2],menemonic,FOpcode);
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
					...
					case 0xC7:
					{
						sprintf(tempMeme,"cmpxchg8b %s",menemonic);
					}
					break;

                    case 0xD7:sprintf(tempMeme,"%s %s,%s",NewSet13[Op&0x0F],regs[RM][reg2],menemonic);break;

                    case 0xD1:case 0xD2:case 0xD3:case 0xD5:case 0xD8:case 0xDF:
                    case 0xD9:case 0xDA:case 0xDB:case 0xDC:case 0xDD:case 0xDE:                    
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet13[Op&0x0F],Regs3DNow[reg2],menemonic);
                    }
                    break;
                    ...
                    case 0xEE:case 0xEF:
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet14[Op&0x0F],Regs3DNow[reg2],menemonic);
                    }
                    break;
                    ...
                    case 0xFD:case 0xFE:
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet15[Op&0x0F],Regs3DNow[reg2],menemonic);
                    }
                    break;
        ...
        */
		...
		switch(Extension) // +/+00/+00000000
		{
			case 00: // No extension of bytes
			{
				...
				else if(reg2==REG_EBP) // Replace EBP with Dword Number
				{
                    ...
                    if(reg1==REG_ESP) // Must Not Be ESP (Index)
                    {
                        ...
                        sprintf(temp,"%s",regs[ADDRM][reg1]);
			...
			{				
                ...
                {                
					SEG=SEG_SS;
                    sprintf(tempMeme,"%s ptr %s:[%s%s%02X]",RSize,segs[SEG],regs[ADDRM][reg2],tempAritmathic,FOpcode);
                }
				else
				    sprintf(tempMeme,"%s ptr %s:[%s%s%s%s%02X]",RSize,segs[SEG],regs[ADDRM][reg1],Aritmathic,regs[ADDRM][reg2],tempAritmathic,FOpcode);
			...
			case 02:// Dword extension
			{
				...
				if(reg1!=REG_ESP) 
                {
                  ...
                  sprintf(tempMeme,"%s ptr %s:[%s%s%s%s%08X]",
					RSize,  // size of register
					segs[SEG], // segment
					regs[ADDRM][reg1],
					Aritmathic,
                    ...
                    sprintf(tempMeme,"%s ptr %s:[%s%s%08X]",
                        RSize,  // size of register
                        segs[SEG], // segment
                        regs[ADDRM][reg2],
                        tempAritmathic,
        ...
        {            
            case 0: // direction (->)
            {
                switch(Op)// Check for all Cases Availble            
                {
                    ...
                    case 0x11: 
                        {
                            if(RepPrefix==1)
                            {
                                sprintf(tempMeme,"movss %s,%s",menemonic,MMXRegs[REG]);
                                strcpy((*Disasm)->Assembly,"");
                                (*Disasm)->OpcodeSize++;
                            }
                            else
                                sprintf(tempMeme,"movups %s,%s",menemonic,MMXRegs[REG]); 
                        }
                        break; // MOVUPS
                    case 0x13: sprintf(tempMeme,"movlps %s,%s",menemonic,MMXRegs[REG]); break; // MOVLPS
                    case 0x17: sprintf(tempMeme,"movhps %s,%s",menemonic,MMXRegs[REG]); break; // MOVHPS
                    case 0x29: sprintf(tempMeme,"movaps %s,%s",menemonic,MMXRegs[REG]); break; // MOVAPS
                    case 0x7E: case 0x7F:sprintf(tempMeme,"%s %s,%s",NewSet7[Op&0x0F],menemonic,Regs3DNow[REG]); break; // MOVD/MOVQ
                    case 0xA3: case 0xAB: 
                    {
                      sprintf(tempMeme,"%s %s,%s",NewSet10[Op&0x0F],menemonic,regs[RM][REG]); 
                    }
                    break;
                    case 0xA4:case 0xAC:
                    {
                      sprintf(tempMeme,"%s %s,%s,%02X",NewSet10[Op&0x0F],menemonic,regs[RM][REG],FOpcode);
                      sprintf(menemonic," %02X",FOpcode);
                      strcat((*Disasm)->Opcode,menemonic);
                      (*Disasm)->OpcodeSize++;
                      (*(*index))++;
                    ...
                    case 0xA5: case 0xAD:
                    {
                      sprintf(tempMeme,"%s %s,%s,cl",NewSet10[Op&0x0F],menemonic,regs[RM][REG]); 
                    }
                    break;
                    ...
                    case 0xB0: case 0xB1: case 0xB3: case 0xBB:
                    {
                      ...
                      sprintf(tempMeme,"%s %s,%s",NewSet11[Op&0x0F],menemonic,regs[RM][REG]);
                    }
                    break;
                    case 0xC0: { RM=REG8; sprintf(tempMeme,"xadd %s,%s",menemonic,regs[RM][REG]); } break; // XADD
                    case 0xC1: sprintf(tempMeme,"xadd %s,%s",menemonic,regs[RM][REG]);              break; // XADD  
                    case 0xE7: sprintf(tempMeme,"%s %s,%s",NewSet14[Op&0x07],menemonic,Regs3DNow[REG]); break;
                }
            ...
            case 1: // direction (<-)
            {
                ...
                {
                    case 0x02: sprintf(tempMeme,"lar %s,%s",regs[RM][REG],menemonic);     break; // LAR
                    case 0x03: sprintf(tempMeme,"lsl %s,%s",regs[RM][REG],menemonic);     break; // LSL
                    case 0x10: 
                        {
                            if(RepPrefix==1) // rep prefix is being used
                            {
                                strcpy((*Disasm)->Assembly,"");
                                (*Disasm)->OpcodeSize++;
                                sprintf(tempMeme,"movss %s,%s",MMXRegs[REG],menemonic);
                            }
                            else
                                sprintf(tempMeme,"movups %s,%s",MMXRegs[REG],menemonic);  
                        }
                        break; // MOVUPS
                    case 0x12: sprintf(tempMeme,"movlps %s,%s",MMXRegs[REG],menemonic);   break; // MOVLPS
                    case 0x14: sprintf(tempMeme,"unpcklps %s,%s",MMXRegs[REG],menemonic); break; // UNPCKLPS
                    case 0x15: sprintf(tempMeme,"unpckhps %s,%s",MMXRegs[REG],menemonic); break; // UNPCKHPS
                    case 0x16: sprintf(tempMeme,"movhps %s,%s",MMXRegs[REG],menemonic);   break; // MOVHPS
                    case 0x18:
                    {
                      sprintf(tempMeme,"%s,%s",NewSet3[REG],menemonic);
                    ...
                    break;
                    case 0x28: sprintf(tempMeme,"movaps %s,%s",MMXRegs[REG],menemonic);   break; // MOVAPS
                    
                    case 0x2A: case 0x2C: case 0x2D:case 0x2E: case 0x2F: 
                    {
                       BYTE R=((Op&0x0F)-0x08);
                       if(R==4 || R==5)
                            sprintf(temp,"%s",Regs3DNow[REG]); // 3DNow! Regs
                       else
                            sprintf(temp,"%s",MMXRegs[REG]); // MMX Regs
                    ...
                    case 0x48:case 0x49:case 0x4A:case 0x4B:case 0x4C:case 0x4D:case 0x4E:case 0x4F:
                    {
                      sprintf(tempMeme,"%s %s,%s",NewSet5[Op&0x0F],regs[RM][REG],menemonic); 
                    }
                    break;
                    ...
                    case 0x58:case 0x59:case 0x5C:case 0x5D:case 0x5E:case 0x5F:
                    {
                        if(RepPrefix==1)
                        {
                            ...
                            sprintf(tempMeme,"%s %s,%s",NewSet6[Op&0x0F],MMXRegs[REG],menemonic); 
                    }
                    break;
                    ...
                    case 0x68:case 0x69:case 0x6A:case 0x6B:case 0x6E:case 0x6F:
                    {
                      sprintf(tempMeme,"%s %s,%s",NewSet7[Op&0x0F],Regs3DNow[REG],menemonic); 
                    }
                    break;
                    
                    case 0x70:
                    {
                      sprintf(tempMeme,"%s %s,%s,%02X",NewSet8[Op&0x0F],Regs3DNow[REG],menemonic,FOpcode);
                      sprintf(menemonic," %02X",FOpcode);
                      strcat((*Disasm)->Opcode,menemonic);
                      (*Disasm)->OpcodeSize++;
                      (*(*index))++;
                    ...
                    case 0x74:case 0x75:case 0x76: // MIX
                    {
                       sprintf(tempMeme,"%s %s,%s",NewSet8[Op&0x0F],Regs3DNow[REG],menemonic); 
                    }
                    break;
                    ...
                    break;
                    case 0xAF:sprintf(tempMeme,"%s %s,%s",NewSet10[Op&0x0F],regs[RM][REG],menemonic);break;

                    case 0xB2:case 0xB4:case 0xB5:
                    case 0xB6:case 0xB7:case 0xBC: 
                    case 0xBD:case 0xBE:case 0xBF:
                    {
                     sprintf(tempMeme,"%s %s,%s",NewSet11[Op&0x0F],regs[RM][REG],menemonic);
                    }
                    break;
                    ...
                    case 0xC2:
                    {
                      if(FOpcode<8) // Instructions here
                      {
                          if(RepPrefix==1) // Rep Prefix is being used
                          {
                              sprintf(tempMeme,"%s %s,%s",NewSet12Ex[FOpcode],MMXRegs[REG],menemonic);
                              strcpy((*Disasm)->Assembly,"");
                              (*Disasm)->OpcodeSize++;
                          }
                          else
                              sprintf(tempMeme,"%s %s,%s",NewSet12[FOpcode],MMXRegs[REG],menemonic);
                      }
                      else
                      {
                          if(RepPrefix==1) // Rep Prefix is being used
                          {
                              sprintf(tempMeme,"cmpss %s,%s,%02X",MMXRegs[REG],menemonic,FOpcode);
                              strcpy((*Disasm)->Assembly,"");
                              (*Disasm)->OpcodeSize++;
                          }
                          else
                              sprintf(tempMeme,"cmpps %s,%s,%02X",MMXRegs[REG],menemonic,FOpcode);
                      }     
                      sprintf(menemonic," %02X",FOpcode);
                      strcat((*Disasm)->Opcode,menemonic);
                      (*Disasm)->OpcodeSize++;
                    ...
                    case 0xC4:
                    {
                        sprintf(tempMeme,"pinsrw %s,%s,%02X",Regs3DNow[REG],menemonic,FOpcode);                        
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0xC5:
                    {
                        sprintf(tempMeme,"pextrw %s,%s,%02X",regs[RM][REG],menemonic,FOpcode);                        
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0xC6:
                    {
                        sprintf(tempMeme,"shufps %s,%s,%02X",MMXRegs[REG],menemonic,FOpcode);                        
                        sprintf(menemonic," %02X",FOpcode);
                        strcat((*Disasm)->Opcode,menemonic);
                        (*Disasm)->OpcodeSize++;
                        (*(*index))++;
                    ...
                    case 0xD7:sprintf(tempMeme,"%s %s,%s",NewSet13[Op&0x0F],regs[RM][REG],menemonic); break;
                    
                    case 0xD1:case 0xD2:case 0xD3:case 0xD5:case 0xD8:case 0xDF:
                    case 0xD9:case 0xDA:case 0xDB:case 0xDC:case 0xDD:case 0xDE:                    
                    {
                        sprintf(tempMeme,"%s %s,%s",NewSet13[Op&0x0F],Regs3DNow[REG],menemonic);
                    }
                    break;
                    ...
                    case 0xEE:case 0xEF:
                    {
                      sprintf(tempMeme,"%s %s,%s",NewSet14[Op&0x0F],Regs3DNow[REG],menemonic);
                    }
                    break;
                    ...
                    case 0xFD:case 0xFE:
                    {
                      sprintf(tempMeme,"%s %s,%s",NewSet15[Op&0x0F],Regs3DNow[REG],menemonic);
                    }
                    break;
                }
