...
bool CProtect::ReadMainFile(char* name) // OK
{
	...
	{
		//((BYTE*)&this->m_MainInfo)[n] -= (BYTE)(0xCC^HIBYTE(n));
		//((BYTE*)&this->m_MainInfo)[n] += (BYTE)(0xBB^HIBYTE(n));
		//((BYTE*)&this->m_MainInfo)[n] ^= (BYTE)(0xAA^LOBYTE(n));
		((BYTE*)&this->m_MainInfo)[n] -= (BYTE)(0xAF^HIBYTE(n));
		((BYTE*)&this->m_MainInfo)[n] += (BYTE)(0x2E^HIBYTE(n));
		((BYTE*)&this->m_MainInfo)[n] ^= (BYTE)(0xBD^LOBYTE(n));
	}
...
void CProtect::CheckInstance() // OK
{
	...
	wsprintf(buff,"MU_MAIN_S3JPN_%s",SERVER_ADRESS);
