...
ANITHACK_PROCDUMP g_ProcessesDumps[MAX_PROCESS_DUMP] = 
{
	...
	{0x437D2E, {0xFF, 0x25, 0x00, 0x20, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},"Easy As MuPie"}, //Easy As MuPie
	...
	{0x405A98, {0xE8 ,0x03 ,0x3D ,0x00 ,0x00 ,0xE9 ,0x79 ,0xFE ,0xFF ,0xFF ,0x8B ,0xFF ,0x55 ,0x8B ,0xEC ,0x8B ,0x45 ,0x08 ,0xA3 ,0x9C ,0xC0 ,0x41 ,0x00 ,0x5D ,0xC3 ,0x8B ,0xFF ,0x55 ,0x8B ,0xEC ,0x81 ,0xEC},"MU AutoClicker (Elite) V3.0400.1202.10003"},	//MU AutoClicker (Elite) V3.0400.1202.10003
	//by DangeR 2015 =)
	{0x00401000, {0xEB, 0x10 ,0x66 ,0x62 ,0x3A ,0x43 ,0x2B ,0x2B ,0x48 ,0x4F ,0x4F ,0x4B ,0x90 ,0xE9 ,0xF4 ,0x61 ,0x4E ,0x00 ,0xA1 ,0xE7 ,0x61 ,0x4E ,0x00 ,0xC1 ,0xE0 ,0x02 ,0xA3 ,0xEB ,0x61 ,0x4E ,0x00 ,0x52},"ollydbg"},	//MU AutoClicker (Elite) V3.0400.1202.10003
	{0x00413380, {0x60, 0xBE ,0x00 ,0x00 ,0x41 ,0x00 ,0x8D ,0xBE ,0x00 ,0x10 ,0xFF ,0xFF ,0x57 ,0x83 ,0xCD ,0xFF ,0xEB ,0x10 ,0x90 ,0x90 ,0x90 ,0x90 ,0x90 ,0x90 ,0x8A ,0x06 ,0x46 ,0x88 ,0x07 ,0x47 ,0x01 ,0xDB},"[Hasty MU v0.3] - Plus Thai"},	//MU AutoClicker (Elite) V3.0400.1202.10003
	//NEW
	{0x00452CCC,{0x55, 0x8b, 0xec, 0x83, 0xc4, 0xf4, 0x53, 0xb8, 0x64, 0x2b, 0x45, 0x00, 0xe8, 0xaf, 0x26, 0xfb, 0xff, 0x8b, 0x1d, 0x34, 0x3f, 0x45, 0x00, 0x8b, 0x03, 0xe8, 0x0e, 0xa0, 0xfd, 0xff, 0x8b, 0x0d},"HexEdit"},
	{0x00402BBA,{0xb8, 0x2c, 0xb0, 0x42, 0x00, 0x50, 0x64, 0xff, 0x35, 0x00, 0x00, 0x00, 0x00, 0x64, 0x89, 0x25, 0x00, 0x00, 0x00, 0x00, 0x33, 0xc0, 0x89, 0x08, 0x50, 0x45, 0x43, 0x6f, 0x6d, 0x70, 0x61, 0x63},"Crc32"},
	{0x00442C60,{0x55, 0x8b, 0xec, 0x6a, 0xff, 0x68, 0x40, 0x5d, 0x48, 0x00, 0x68, 0xd0, 0xa7, 0x44, 0x00, 0x64, 0xa1, 0x00, 0x00, 0x00, 0x00, 0x50, 0x64, 0x89, 0x25, 0x00, 0x00, 0x00, 0x00, 0x83, 0xc4, 0xa4},"BmdDec"},
	{0x005231B4,{0x55, 0x8b, 0xec, 0x83, 0xc4, 0xf0, 0xb8, 0x68, 0xb6, 0x51, 0x00, 0xe8, 0x70, 0x75, 0xee, 0xff, 0xa1, 0xdc, 0x8a, 0x52, 0x00, 0x8b, 0x00, 0xe8, 0x98, 0xc9, 0xfe, 0xff, 0xa1, 0xdc, 0x8a, 0x52},"BMD FIXER"},
...
//========================================================================
bool TitleWindowCheck(LPCSTR WindowTitle){
	HWND WinTitle = FindWindowA(NULL,WindowTitle);
	if( WinTitle > 0)
	{
		SendMessage(WinTitle, WM_CLOSE,0,0);  //CLOSE HACK WINDOW
		//g_AntiCheatPlus.Write(CLASS_WINDOW_TEXT,WindowTitle);
...
void TitleWindowMain()
{
	...
	TitleWindowCheck("OllyDbg - [CPU]");
	TitleWindowCheck("LibHook v0.1.2.1");
}
...
{
	CreateThread(NULL,NULL,LPTHREAD_START_ROUTINE(YProcessScan),NULL,0,0);
}
//========================================================================
#define WINNT 1
...
void HT()
{
	LogsAdded ("[Closed]: Hide windows detected!" );
		...
		if(Process32First(hProcessSnap, &pe32))
		{
			do
			{
				...
				if(hProcess != NULL)
				{
					...
					{
						//MessageBoxA(FindWindowA(0,"Phoenix Guard"), "Found hack software in your system.\n\nHint: Close all illegal programs and run application again.", "Phoenix MuGuard v2.1", MB_OK | MB_ICONSTOP);
...
void Injetado()
{
	...
	LogsAdded ("[Closed]: Dll injection!" );
...
//cout << *(unsigned long*)QueryPerformanceCounter <<endl;
	...
	{
		LogsAdded ("[Closed]: Speed Hack detected!" );
...
void tickplash()
{
	...
	LogsAdded ("[Closed]: Speed Hack or System Freeze detected!" );
