...
bool CItemValue::GetItemValue(DWORD address,int* value) // OK
{
	...
	for(int n=0;n<MAX_ITEM_VALUE_INFO;n++)
	{
		...
		if(lpInfo->ItemIndex == *(WORD*)(address+0x00))
		{
			...
			{
				if(lpInfo->Grade == -1 || lpInfo->Grade == GET_ITEM_OPT_EXC(*(BYTE*)(address+0x1A)))
				{
					if(gItemStack.GetItemMaxStack(lpInfo->ItemIndex,lpInfo->Level) == 0 || lpInfo->ItemIndex == ITEM(4,7) || lpInfo->ItemIndex == ITEM(4,15))
					{
						(*value) = lpInfo->Value;
						//(*value) = lpInfo->Value1;
						return 1;
					...
					{
						(*value) = (int)(lpInfo->Value**(BYTE*)(address+0x16));
						//(*value) = (int)(lpInfo->Value1**(BYTE*)(address+0x16));
						return 1;
					}
				}
			}
...
bool CItemValue::GetItemValue1(DWORD address,int* value1) // OK
{
	//if(gCustomJewel.CheckCustomJewelByItem(*(WORD*)(address+0x00))!=0)
	//{
	//	(*value) = gCustomJewel.GetCustomJewelSalePrice(*(WORD*)(address+0x00));
		...
		if(lpInfo->ItemIndex == *(WORD*)(address+0x00))
		{
			if(lpInfo->Level == -1 || lpInfo->Level == GET_ITEM_OPT_LEVEL(*(DWORD*)(address+0x04)))
			{
				if(lpInfo->Grade == -1 || lpInfo->Grade == GET_ITEM_OPT_EXC(*(BYTE*)(address+0x1A)))
				{
					if(gItemStack.GetItemMaxStack(lpInfo->ItemIndex,lpInfo->Level) == 0 || lpInfo->ItemIndex == ITEM(4,7) || lpInfo->ItemIndex == ITEM(4,15))
					{
						(*value1) = lpInfo->Value1;
						//(*value) = lpInfo->Value1;
						return 1;
					}
					else
					{
						(*value1) = (int)(lpInfo->Value1**(BYTE*)(address+0x16));
						//(*value) = (int)(lpInfo->Value1**(BYTE*)(address+0x16));
						return 1;
					}
				}
