...
int CCommandInterface::FilterSendPacket(CPacket& pkt, CFilterContext& context)
{
	...
	if (pktCopy == CCharacterSayPacket::Type() 
			|| pktCopy == CCharacterSayPacket2::Type()
			|| pktCopy == CCharacterSayPacket3::Type()
			|| pktCopy == CCharacterWhisperPacket::Type())
	{
		char* szText = (char*)buf + 13;

		if (strlen(szText) > 0 && szText[0] == '/' && szText[1] == '/')
		{
